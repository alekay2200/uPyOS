...
def _trigger(rotary_instance):
    if rotary_instance._listener is not None:
        rotary_instance._listener(None)

# For multiple listeners, not work with class methods
# def _trigger(rotary_instance):
#     for l in rotary_instance._listener:
#         l(None)
...
class Rotary(object):
    ...
    def _process_rotary_pins(self, pin):
        ...
        try:
            # For multiple listeneres, not work with class methods
            # if old_value != self._value and len(self._listener) != 0:
            #     micropython.schedule(_trigger, self)
            if old_value != self._value and self._listener is not None:
                micropython.schedule(_trigger, self)
        except:
            pass
